...
export const api = {
  ...
  stats: (apiKey: string, clientId: string) => request<Stats>(`/stats/${clientId}`, apiKey),

  listRules: (apiKey: string) =>
    request<{ rules: Rule[] }>('/rules/list', apiKey).then((r) => r.rules),
  createRule: (apiKey: string, rule: Rule) =>
    request<Rule>('/rules', apiKey, { method: 'POST', body: JSON.stringify(rule) }),
  deleteRule: (apiKey: string, name: string) =>
    request<void>(`/rules/${encodeURIComponent(name)}`, apiKey, { method: 'DELETE' }),

  listExemptions: (apiKey: string) =>
    request<{ exemptions: Exemption[] }>('/exemptions/list', apiKey).then((r) => r.exemptions),
  createExemption: (apiKey: string, exemption: Exemption) =>
    request<Exemption>('/exemptions', apiKey, { method: 'POST', body: JSON.stringify(exemption) }),
  deleteExemption: (apiKey: string, identifier: string) =>
    request<void>(`/exemptions/${encodeURIComponent(identifier)}`, apiKey, { method: 'DELETE' }),
}
