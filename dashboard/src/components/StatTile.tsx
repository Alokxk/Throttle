...
export function StatTile({ label, value, icon, accent = 'default' }: StatTileProps) {
  return (
    <div className="rounded-xl border border-neutral-200 bg-white p-4 shadow-sm shadow-neutral-900/[0.02] dark:border-neutral-800 dark:bg-neutral-900">
      <div className="flex items-center gap-2">
        <span className={`flex h-7 w-7 items-center justify-center rounded-lg ${ICON_BG[accent]}`}>
          {icon}
        </span>
      ...
      {value === null ? (
        ...
        <p className={`mt-2 text-2xl font-semibold tabular-nums ${ACCENT_CLASSES[accent]}`}>
          {value}
        </p>
      )}
    </div>
